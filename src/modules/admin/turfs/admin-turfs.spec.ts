...
describe('AdminTurfs', () => {
  ...
  const mockPrisma = {
    turf: {
      ...
      findMany: jest.fn().mockResolvedValue([{ id: 'turf1', name: 'Turf A', status: 'ACTIVE' }]),
      findUnique: jest.fn().mockResolvedValue({ id: 'turf1', name: 'Turf A', status: 'ACTIVE', deletedAt: null }),
  ...
  it('should activate turf', async () => {
    const res = await controller.activateTurf('turf1', { adminId: 'admin1' }, { ip: '127.0.0.1' } as any);
    expect(res.success).toBe(true);
  });

  it('should deactivate turf', async () => {
    const res = await controller.deactivateTurf('turf1', { adminId: 'admin1' }, { ip: '127.0.0.1' } as any);
    expect(res.success).toBe(true);
  });

  it('should suspend turf', async () => {
    const res = await controller.suspendTurf('turf1', { reason: 'guidelines' }, { adminId: 'admin1' }, { ip: '127.0.0.1' } as any);
    expect(res.success).toBe(true);
  });

  it('should feature turf', async () => {
    const res = await controller.featureTurf('turf1', { adminId: 'admin1' }, { ip: '127.0.0.1' } as any);
    expect(res.success).toBe(true);
  });

  it('should unfeature turf', async () => {
    const res = await controller.unfeatureTurf('turf1', { adminId: 'admin1' }, { ip: '127.0.0.1' } as any);
    expect(res.success).toBe(true);
  });
});
