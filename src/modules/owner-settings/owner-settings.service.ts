...
export class OwnerSettingsService {
  ...
  async updatePaymentSettings(authId: string, dto: UpdatePaymentSettingsDto) {
    ...
    if (bankHolderName.length < 3 || bankHolderName.length > 100) {
      throw new BadRequestException(
        'Account holder name must be between 3 and 100 characters',
      );
    }
    if (!/^[a-zA-Z\s.]+$/.test(bankHolderName)) {
      throw new BadRequestException(
        'Account holder name can only contain alphabets, spaces, and dots',
      );
    }
    if (/[<>'";]|--/.test(bankHolderName)) {
      throw new BadRequestException(
        'Account holder name cannot contain SQL/XSS special characters',
      );
    }
    ...
    if (bankName.length < 3 || bankName.length > 100) {
      throw new BadRequestException(
        'Bank name must be between 3 and 100 characters',
      );
    }
    if (!/^[a-zA-Z\s&]+$/.test(bankName)) {
      throw new BadRequestException(
        'Bank name can only contain alphabets, spaces, and &',
      );
    }
    if (/[<>'";]|--/.test(bankName)) {
      throw new BadRequestException(
        'Bank name cannot contain SQL/XSS special characters',
      );
    }
    ...
    if (new Set(accountNumber).size === 1) {
      throw new BadRequestException(
        'Account number cannot consist of identical digits',
      );
    }
    if (isSequential(accountNumber)) {
      throw new BadRequestException(
        'Account number cannot be a sequential sequence',
      );
    }
    if (isRepeatedPattern(accountNumber)) {
      throw new BadRequestException(
        'Account number cannot contain repeated patterns',
      );
    }
    ...
    if (accountNumber !== confirmAccountNumber) {
      throw new BadRequestException(
        'Confirm account number must exactly match account number',
      );
    }
  ...
  async createMaintenanceBlock(ownerAuthId: string, dto: CreateMaintenanceDto) {
    ...
    if (dto.date) {
      ...
      throw new BadRequestException(
        'Please provide either date, dates, or a startDate and endDate range',
      );
    }
