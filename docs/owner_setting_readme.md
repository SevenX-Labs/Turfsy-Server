...
_(Note: `/payment` and `/payout` point to the same underlying owner settings flow. Payment + payout preferences are now stored in the dedicated `owner-settings` model/table.)_

### Get Payment Settings

- **URL:** `GET /api/v3/owner-settings/payment` _(or `/payout`)_
...
- **URL:** `PATCH /api/v3/owner-settings/payment` _(or `/payout`)_
...
Notification preferences are stored in dedicated owner settings storage.
...
- **Response:**
  ...
  {
    "success": true,
    "message": "Password change is not applicable for OTP-based login. Use secure phone change flow for credential updates."
  }
  ```
